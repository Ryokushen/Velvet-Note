...
export const FAMILY: Record<Family, { tint: string; border: string; dot: string; label: string }> = {
  ...
  // Spicy shares woody's tint/border weighting but takes a lighter, hotter
  // oxblood dot so the two families read apart on the calendar — still inside
  // the warm-hue discipline (same red hue, raised chroma/lightness only).
  spicy:    { tint: 'rgba(139,58,58,0.28)',  border: 'rgba(139,58,58,0.55)',  dot: '#B24A46', label: 'Spicy' },
};
