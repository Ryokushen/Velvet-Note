...
**Catalog and Calendar follow-ups (2026-04-24):** `0fca8be` added accord autocomplete and selected-date Calendar wear entry; `ca4222e` added the Kaggle import pipeline and normalized local catalog; `deb5846` wired Add-screen catalog lookup/prefill. `23e9433` added the Parfumo import path, `785737c` persisted selected catalog metadata on shelf entries, `ba9ba7c` moved Add search to the Supabase RPC, and `5c820df` improved note-search ranking plus scrollable results. Barcode scanning, contribution/moderation, and LLM fallback remain Phase 2.
...
Current implementation notes for wear logging, the Calendar tab, accord autocomplete, and the handoff into Phase 2 catalog search.
